...
const (
	// If accepted, Channel will be updated and it will remain open for off-chain transactions.
	ChUpdateTypeOpen ChUpdateType = "open"
	// If accepted, Channel will be updated, it will be settled on-chain and amount for this user will be withdrawn.
	ChUpdateTypeFinal ChUpdateType = "final"
	// Channel has been settled on-chain and amount for this user was withdraw (if error is non empty).
	// The channel is closed and cannot be accessed. Client should unsubscribe from channel updates after this update.
	ChUpdateTypeClosed ChUpdateType = "closed"
)

type (

	// ChStatus is the type of channel update. It can have three values: "open", "final" and "closed".
	ChUpdateType string

...
func Test_Integ_Alice(t *testing.T) {
	// Deploy contracts.
	...
	aliceCfg.IDProviderURL = "../alice/idprovider.yaml"
	aliceCfg.User.CommAddr = "127.0.0.1:3031"
	bobCfg.IDProviderURL = "../bob/idprovider.yaml"
	bobCfg.User.CommAddr = "127.0.0.1:3032"
	...
	t.Run("GetPeerID", func(t *testing.T) {
		t.Run("happy", func(t *testing.T) {
			ownPeerID, err := alice.GetPeerID(perun.OwnAlias)
			require.NoErrorf(t, err, "Bob: GetPeerID")
			t.Logf("ownPeerID: %+v", ownPeerID)
			bobPeerID, err := alice.GetPeerID(bobAlias)
			require.NoErrorf(t, err, "Alice: GetPeerID")
			t.Logf("bobPeerID: %+v", bobPeerID)
		})
	})
...
// This test includes all methods on SessionAPI and ChAPI.
func Test_Integ_Bob(t *testing.T) {
	// Deploy contracts.
	ethereumtest.SetupContractsT(t, ethereumtest.ChainURL, ethereumtest.OnChainTxTimeout)
	...
	t.Run("GetPeerID", func(t *testing.T) {
		t.Run("happy", func(t *testing.T) {
			alicePeerID, err := bob.GetPeerID(aliceAlias)
			require.NoErrorf(t, err, "Bob: GetPeerID")
			t.Logf("alicePeerID: %+v", alicePeerID)
			ownPeerID, err := bob.GetPeerID(perun.OwnAlias)
			require.NoErrorf(t, err, "Bob: GetPeerID")
			t.Logf("ownPeerID: %+v", ownPeerID)
		})
	})
	...
	t.Run("Collaborative channel close", func(t *testing.T) {
		...
		err = bobCh.UnsubChUpdates()
		assert.Error(t, err)
		t.Log(err, "UnsubChUpdates for bob")

		require.EqualError(t, err, perun.ErrChClosed.Error())

		t.Run("Session_Close_NoForce_Sucesss", func(t *testing.T) {
			var openChsInfo []perun.ChInfo
			openChsInfo, err = bob.Close(false)
			require.NoError(t, err)
			require.Len(t, openChsInfo, 0)
		})
	})

...
func (s *session) OpenCh(pctx context.Context, openingBalInfo perun.BalInfo, app perun.App, challengeDurSecs uint64) (
	...
	openedCh := newCh(pch, openingBalInfo.Currency, openingBalInfo.Parts, s.timeoutCfg, challengeDurSecs)
	s.addCh(openedCh)
...
func (s *session) HandleUpdate(chUpdate pclient.ChannelUpdate, responder *pclient.UpdateResponder) {
	...
	notif := perun.ChUpdateNotif{
		UpdateID:       updateID,
		CurrChInfo:     ch.getInfo(),
		ProposedChInfo: chInfo(ch.ID(), ch.parts, ch.currency, chUpdate.State),
		Expiry:         expiry,
	}

...
func (s *session) AddContact(peer perun.Peer) error {
	s.Debugf("\nReceived request: session.AddContact. Params %+v", peer)
	s.Lock()
	defer s.Unlock()
...
func (s *session) GetContact(alias string) (perun.Peer, error) {
	s.Debugf("\nReceived request: session.GetContact. Params %+v", alias)
	s.Lock()
	defer s.Unlock()
...
func (s *session) HandleProposal(chProposal *pclient.ChannelProposal, responder *pclient.ProposalResponder) {
	...
	proposalID := fmt.Sprintf("%x", chProposal.SessID())
	...
	if s.chProposalNotifier == nil {
		...
		s.Logger.Debugf("SDK Callback: Notification cached", notif)
	} else {
		go s.chProposalNotifier(notif)
		s.Logger.Debugf("SDK Callback: Notification sent", notif)
	}
}
...
func (s *session) RespondChProposal(pctx context.Context, chProposalID string, accept bool) error {
	...
	if !ok {
		s.Logger.Info(perun.ErrUnknownProposalID)

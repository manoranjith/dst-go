...
func NewEthereumPaymentClient(cfg Config, user node.User, comm node.CommBackend) (*Client, error) {
...
func initComm(comm node.CommBackend, listenAddr string) (peer.Dialer, peer.Listener, error) {
	dialer := comm.NewDialer()
	listener, err := comm.NewListener(listenAddr)
	return dialer, listener, err
}

func connectToChain(cfg ChainConfig, cred node.Credential) (channel.Funder, channel.Adjudicator, error) {
